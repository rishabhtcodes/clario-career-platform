...
export default function HomePage() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      // Animate loading messages regardless
      messages.forEach((_, i) => {
        setTimeout(() => setStep(i), i * 1000);
      });

      // If Supabase is not configured, redirect to auth after animation
      if (!supabase) {
        console.warn("⚠️ Supabase not configured — redirecting to /auth");
